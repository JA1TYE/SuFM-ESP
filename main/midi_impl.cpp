...
namespace su_midi{
    midi_receiver_impl::midi_receiver_impl(su_synth::synth_controller *synth_inst,std::uint16_t mask):midi_receiver_base(mask){
    ...
    void midi_receiver_impl::pc_handler(std::uint8_t prog_num,std::uint8_t ch){
        synth_inst_->set_timbre(su_synth::PROGRAM_CHANGE,prog_num,ch);
    }
    ...
    void midi_receiver_impl::cc_handler(std::uint8_t control_num,std::uint8_t value,std::uint8_t ch){
        if(control_num == CC_BANK_SELECT_MSB){
            synth_inst_->set_timbre(su_synth::BANK_MSB,value,ch);
        }
        else if(control_num == CC_BANK_SELECT_LSB){
            synth_inst_->set_timbre(su_synth::BANK_LSB,value,ch);
        }
            ...
            synth_inst_->set_modulation(su_synth::MOD_WHEEL,value,ch);
        }
        else if(control_num == CC_FOOT_CONTROLLER_MSB){
            synth_inst_->set_modulation(su_synth::MOD_FOOT,value,ch);
        }
        else if(control_num == CC_BREATH_CONTROLLER_MSB){
            synth_inst_->set_modulation(su_synth::MOD_BREATH,value,ch);
        }
        else if(control_num == CC_DUMPER_PEDAL){
            synth_inst_->set_hold(value,ch);
        }
        else if(control_num == CC_RPN_MSB){
            synth_inst_->set_parameter_number(su_synth::RPN_MSB,value,ch);
        }
        else if(control_num == CC_RPN_LSB){
            synth_inst_->set_parameter_number(su_synth::RPN_LSB,value,ch);
        }
        else if(control_num == CC_NRPN_MSB){
            synth_inst_->set_parameter_number(su_synth::NRPN_MSB,value,ch);
        }
        else if(control_num == CC_NRPN_LSB){
            synth_inst_->set_parameter_number(su_synth::NRPN_LSB,value,ch);
        }
